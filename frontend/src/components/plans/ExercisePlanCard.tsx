...
import { Calendar, ChevronRight, Trash2, Loader2, Dumbbell } from 'lucide-react';
...
export function ExercisePlanCard({ plan, onViewDetails }: Props) {
  const [showConfirm, setShowConfirm] = useState(false);
  const queryClient = useQueryClient();

  const deleteMutation = useMutation({
    mutationFn: async () => {
      await api.delete(`/planner/plan/exercise/delete?exercise_id=${plan.id}`);
    },
    onSuccess: () => {
  ...
  const tags = Object.values(plan.exercises || {})
    .map((day: any) => day.focus)
    .filter(Boolean)
    .slice(0, 3);
    ...
    <div className="rounded-[32px] border border-slate-800 bg-slate-900 p-6 md:p-8 flex flex-col justify-between relative overflow-hidden">
      <div>
        <div className="flex items-start justify-between mb-6">
          <div className="flex items-center gap-4">
             ...
             <div>
               <h3 className="text-xl font-bold text-slate-50">Exercise Plan</h3>
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-8">
            {tags.map((tag: any, idx) => (
              <span key={idx} className="rounded-full bg-slate-800 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-slate-400">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="space-y-4">
        ...
        <PlanFeedbackForm 
          ...
          initialCompletion={plan.completed_percentage}
          plan={plan}
        />
      </div>
